 ...
 * @param end 结束点
 */
const generateBasisCurvePoints = (
  start: Point,
  firstControl: Point,
  secondPoint: Point,
  end: Point,
  ...
  for (let i = 1; i < total; i++) {
    const t = i / total;
    const x = getBasisPoint(t, start.x, firstControl.x, secondPoint.x, end.x);
    const y = getBasisPoint(t, start.y, firstControl.y, secondPoint.y, end.y);
    points.push({
      x: x,
      y: y
    });
...
export const calcCurvePoint = (
  ...
  for (let i = 2; i <= control_points.length; i++) {
    ...
    // 计算控制点
    const first_point = getFirstPoint(point0, point1, point2, k);
    const second_point = getSecondPoint(point1, point2, currentPoint, k);
    generateBasisCurvePoints(
      startPoint,
      first_point,
      second_point,
      point2,
