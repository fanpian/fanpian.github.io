...
// CONCATENATED MODULE: ./src/arrayToTree/DefaultOption.ts


/**
 * 默认配置项类
 */
var DefaultOption_DefaultOption = function DefaultOption() {
  ...
  this.childTo = 'children';
  /** 数据源父节点主键 */

  this.parentFrom = 'parentId';
  /** 导出数据父节点主键 */

  this.parentTo = 'parentId';
  /** 在导出数据时,是否删除空的子节点字段 */

  this.deleteEmptyChildrenAttr = false;
  /** 删除指定的节点.删除父节点后，子节点因没有父节点，将无法导出 */

  this.deleteNodes = new Array();
  /** 标记哪些节点需要禁用 */

  this.disabledNodes = new Array();
  /**
   * 根节点的父级主键值,默认值是[undefined, null, 0];
   * 当父节点的值是其中一个，那么这个节点为根节点
   */

  this.rootParentVal = [undefined, null, 0];
};
// CONCATENATED MODULE: ./src/arrayToTree/ToTree.ts
...
var ToTree_extendArray = function extendArray(source, option) {
  var filterDelete = source.filter(function (item) {
    return option.deleteNodes.some(function some(nodeKey) {
      return nodeKey !== item[option.idFrom];
    });
  });
  return filterDelete.map(function (item) {
    var target = Object(lodash["extend"])({}, item);
    ...
    var disabledItem = option.disabledNodes.find(function find(node) {
...
var toTree = function toTree(source, parentNode, option) {
  var childrens = source.filter(function (filter) {
    return filter[option.parentTo] === parentNode[option.idTo];
  });
  childrens.map(function (children) {
    var hasChildren = source.some(function (item) {
...
var ToTree_arrayToTree = function arrayToTree(source, option) {
  var defaultOption = new DefaultOption_DefaultOption();
  var options = Object(lodash["extend"])(defaultOption, option);
  var node = ToTree_extendArray(source, options);
  return node.filter(function (filter) {
...
/* harmony default export */ var ToTree = (ToTree_arrayToTree);
// EXTERNAL MODULE: ./src/arrayToTree/IOption.ts
var IOption = __webpack_require__("2da1");

// CONCATENATED MODULE: ./src/arrayToTree/index.ts



/* harmony default export */ var src_arrayToTree = (ToTree);
// EXTERNAL MODULE: ./node_modules/core-js/modules/es6.typed.uint8-array.js
var es6_typed_uint8_array = __webpack_require__("34ef");

// CONCATENATED MODULE: ./src/base64Library.ts
...
var toUint8Array = function toUint8Array(source) {
  ...
  if (subMark) {
    var index = copy.indexOf(base64Mark);
  ...
  var decodeStr = window.atob(copy);
  var outputArray = new Uint8Array(decodeStr.length);
...
// CONCATENATED MODULE: ./src/arrayBufferLibrary.ts
 ...
 */
var toBase64 = function toBase64(source) {
...
/******/ })["default"];
//# sourceMappingURL=js-handle.common.js.map